...
from .backbones.resnet_ibn_b import resnet101_ibn_b, resnet152_ibn_b
...
class Backbone(nn.Module):
    def __init__(self, num_classes, cfg):
        ...
        elif model_name == 'resnet152_ibn_b':
            self.in_planes = 2048
            self.base = resnet152_ibn_b(last_stride)
            print('using resnet152_ibn_b as a backbone')
        else:
    ...
    def load_param_classifier(self, trained_path):
        param_dict = torch.load(trained_path)
        for i in param_dict:
            self.state_dict()[i.replace('module.', '')].copy_(param_dict[i])
        print('Loading pretrained model from {}'.format(trained_path))
