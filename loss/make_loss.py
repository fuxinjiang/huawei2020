        ...
        if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet_center':
            ...
            if cfg.MODEL.IF_LABELSMOOTH == 'on':
                return cfg.MODEL.ID_LOSS_WEIGHT * id_loss_func(score, target) + \
                       ...
                       cfg.SOLVER.CENTER_LOSS_WEIGHT * center_criterion(feat.float(), target.float())
            else:
                return cfg.MODEL.ID_LOSS_WEIGHT * id_loss_func(score, target) + \
                       cfg.MODEL.TRIPLET_LOSS_WEIGHT * triplet(feat, target)[0] + \
                       cfg.SOLVER.CENTER_LOSS_WEIGHT * center_criterion(feat.float(), target.float())
            ...
            if cfg.MODEL.IF_LABELSMOOTH == 'on':
                return cfg.MODEL.ID_LOSS_WEIGHT * id_loss_func(score, target) + \
                       cfg.MODEL.TRIPLET_LOSS_WEIGHT * triplet(feat, target)[0], id_loss_func(score, target), triplet(feat, target)[0]
            else:
                return cfg.MODEL.ID_LOSS_WEIGHT * id_loss_func(score, target) + \
                       cfg.MODEL.TRIPLET_LOSS_WEIGHT * triplet(feat, target)[0]
        else:
